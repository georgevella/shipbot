...
namespace Shipbot.Controller.Core.ApplicationSources.Sync
{
    public class HelmApplicationSyncService : IApplicationSourceSyncService 
    {
        ...
        public async Task<HelmApplicationSourceDetails> BuildApplicationSourceDetails(
            ...
            )
        {
            ...
            return new HelmApplicationSourceDetails(
...
//            while ((deploymentUpdate = await _deploymentService.GetNextPendingDeploymentUpdate(application)) != null) 
            {
                ...
                if (!helmApplicationSourceDetails.Files.TryGetValue(deploymentUpdate.Image, out FileInfo file))
                {
                ...
                manifestsChanged = await UpdateDeploymentManifests(
                    helmApplicationSourceDetails, 
                    deploymentUpdate, 
                    file,
                    helmApplicationSourceDetails.Tags.TryGetAndReturn(deploymentUpdate.Image) ?? "n/a"
                    );
...
//                }
        ...
        private async Task<bool> UpdateDeploymentManifests(
            HelmApplicationSourceDetails applicationSourceDetails,
            DeploymentUpdate deploymentUpdate,
            FileSystemInfo file,
...
//            }
            ...
            using (var gitRepository = new Repository(applicationSourceDetails.GitRepositoryPath.FullName))
            {

                var gitFilePath =
                    Path.GetRelativePath(applicationSourceDetails.GitRepositoryPath.FullName, file.FullName);
    ...
    public class HelmApplicationSourceDetails
    {
        public DirectoryInfo GitRepositoryPath { get; }
        public IDictionary<Image, string> Tags { get; }
        public IDictionary<Image, FileInfo> Files { get; }

        internal HelmApplicationSourceDetails(
            IDictionary<Image, string> tags, 
            IDictionary<Image, FileInfo> files,
            DirectoryInfo gitRepositoryPath)
        {
